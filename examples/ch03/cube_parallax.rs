...
fn create_vertices(ul: f32, vl: f32) -> (Vec<Vertex>, Vec<u16>) {
    let (pos, _, normal, uv, ind, _) = vd::create_cube_data(2.5);
...
impl State {
    async fn new(window: &Window, sample_count: u32) -> Self {
        ...
        let fs_shader = init
            .device
            .create_shader_module(wgpu::include_wgsl!("parallax_frag.wgsl"));

        // uniform data
        let camera_position = (2.0, 2.5, 4.0).into();
        ...
        let material = [0.1 as f32, 0.4, 0.1, 25.0, 1.0, 0.01];
        init.queue
        ...
        let img_files = vec![
            "assets/brick1.png",
            "assets/brick1-normal.png",
            "assets/brick1-height.png",
        ];
...
fn main() {
    ...
    let window = winit::window::WindowBuilder::new()
        .build(&event_loop)
        .unwrap();
    window.set_title(&*format!("{}", "cube_parallax"));
