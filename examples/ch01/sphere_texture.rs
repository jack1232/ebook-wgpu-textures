...
fn create_vertices() -> (Vec<Vertex>, Vec<u16>) {
    let (pos, norm, uv, ind, _) = vd::create_sphere_data(2.0, 20, 32);
...
fn image_file_map(n: u32) -> Option<&'static str> {
    ...
    d.insert(0, "assets/earth.png");
    d.insert(1, "assets/brick.png");
    d.insert(2, "assets/grass.png");
    d.insert(3, "assets/wood.png");
    d.insert(4, "assets/marble.png");

    d.get(&n).cloned()
}

const ADDRESS_MODE: wgpu::AddressMode = wgpu::AddressMode::ClampToEdge;
...
impl State {
    async fn new(window: &Window, sample_count: u32) -> Self {
        ...
        let material = [0.1 as f32, 0.4, 0.4, 100.0];
        init.queue
        ...
        match event {
            WindowEvent::KeyboardInput {
                ...
                VirtualKeyCode::Space => {
                    self.image_selection = (self.image_selection + 1) % 5;
...
fn main() {
    ...
    let window = winit::window::WindowBuilder::new()
        .build(&event_loop)
        .unwrap();
    window.set_title(&*format!("{}", "sphere_texture"));
