...
fn create_vertices(ul: f32, vl: f32) -> (Vec<Vertex>, Vec<u16>) {
    let (pos, _, norm, uv, ind, _) = vd::create_cube_data(2.5);
...
fn image_file_map(n: u32) -> Option<&'static str> {
    ...
    d.insert(1, "assets/grass.png");
    d.insert(2, "assets/wood.png");
    d.insert(3, "assets/marble.png");
...
impl State {
    async fn new(window: &Window, sample_count: u32) -> Self {
        ...
        // uniform data
        let camera_position = (2.5, 2.0, 3.0).into();
        ...
        match event {
            WindowEvent::KeyboardInput {
                ...
                VirtualKeyCode::Space => {
                    self.image_selection = (self.image_selection + 1) % 4;
...
fn main() {
    ...
    let window = winit::window::WindowBuilder::new()
        .build(&event_loop)
        .unwrap();
    window.set_title(&*format!("{}", "cube_texture"));
